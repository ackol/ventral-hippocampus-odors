...
%       
%       (3) [IDxxx]_paired_full_session_inst_velocity.mat (containing
%       "???" variable)
%
%
%   Outputs:
%       (1) [IDxxx]_locomotion_grouped_by_odor_trial.mat, containing the
%       following variables:
%           locomotionMatrix
%           mouseLabel
%           timeBefore
...
% Note: This script is adapted from align_sorted_spikes_to_odor_delivery.m
% Version 1.0
...
[file, path] = uigetfile(".mat","Select _paired_full_session_inst_velocity.mat variable for " + mouseLabel + ".",baseDir);
tic
disp("Loading velocity structure...")
load(fullfile(path,file));
toc            
...
% GOT ONLY TO HERE WITH REVISIONS!!
clear trialCountByOdor odorantNum onPoint

% Preallocate memory for new data structure
runningRaster = nan(nOdors, nTrialsPerOdor, 1, pointsBefore+pointsDuring+pointsAfter); % Create empty 3D matrix for storing per-trial running traces, grouped by odorant identity

% Populate new data structure, organizing locomotion trace by odor identity,
% trial number
tic
disp("Grouping running data by trial...")
...
for iPresentation = 1:nOdorPresentations
    ...
    runningRaster(odorantNum, trialCountByOdor(1,odorantNum), :, :) = wheelVelocity(:, (onPoint-pointsBefore):(onPoint+pointsDuring+pointsAfter-1));
end
fprintf('\n')
toc
%%
...
tic
disp("Saving locomotion_grouped_by_odor_trial data structure to .mat file.")
save(baseOutputPath + "\" + mouseLabel + "" + "_locomotion_grouped_by_odor_trial.mat","runningRaster", "mouseLabel","timeBefore","timeDuring","timeAfter","sampFreq","nOdors","nTrialsPerOdor","odorIdentities",'-v7.3');
toc
