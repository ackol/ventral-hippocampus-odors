% align_sorted_spikes_to_odor_delivery_all_mice.m - This code is designed to import 
...
%       (1) miceToAnalyze.mat
%
%       (2) folder containing [IDxxx]_full_session_odor_sequence_matrix.mat 
%       (containing "odorSeqMat" variable and others) for all mice
%       
%       (3) folder containing [IDxxx]_D[x]_full_session_pid_data.mat 
%       (containing "odorSetInfo" variable) for all mice
%       
%       (3) folder containing [IDxxx]_aligned_phy_unit_data.mat (containing
%       "alignedUnitDataStruct" variable) for all mice
...
%
%   Outputs:
%       (1) One per mouse: [IDxxx]_spikes_grouped_by_odor_trial.mat, 
...
% Set address of folder containing odor delivery timing for all mice
% (i.e. folder of [IDxxx]_full_session_odor_sequence_matrix.mat) 
pathToOdorDelivery = uigetdir('',"Select folder containing [IDxxx]_full_session_odor_sequence_matrix.mat for all mice");
odorDeliveryFiles = dir(fullfile(pathToOdorDelivery,'*.mat'));
if ~(length(odorDeliveryFiles) == nMice)
    error("Odor Delivery Data are missing for some mice.")
end
% extract mouse ID and save it to a field within the files struct
filenames = {odorDeliveryFiles.name};
mouseIDs = string(extractBefore(filenames,"_"));
for iFile = 1:numel(odorDeliveryFiles)
    odorDeliveryFiles(iFile).mouseID = mouseIDs(iFile);
end

% Set address of folder containing odorant identities for all mice
% (i.e. folder of [IDxxx]_full_session_odor_sequence_matrix.mat) 
pathToOdorIDs = uigetdir('',"Select folder containing [IDxxx]_full_session_odor_sequence_matrix.mat for all mice");
odorIDFiles = dir(fullfile(pathToOdorIDs,'*.mat'));
if ~(length(odorIDFiles) == nMice)
    error("Odor Identity Data are missing for some mice.")
end
% extract mouse ID and save it to a field within the files struct
filenames = {odorIDFiles.name};
mouseIDs = string(extractBefore(filenames,"_"));
for iFile = 1:numel(odorIDFiles)
    odorIDFiles(iFile).mouseID = mouseIDs(iFile);
end

% Set address of folder containing load alignedUnitDataStruct variable
% for all mice
pathToAlignedUnits = uigetdir('',"Select folder containing [IDxxx]_aligned_phy_unit_data.mat for all mice");
alignedUnitsFiles = dir(fullfile(pathToAlignedUnits,'*.mat'));
if ~(length(alignedUnitsFiles) == nMice)
    error("Aligned Units Data are missing for some mice.")
end
% extract mouse ID and save it to a field within the files struct
filenames = {alignedUnitsFiles.name};
mouseIDs = string(extractBefore(filenames,"_"));
for iFile = 1:numel(alignedUnitsFiles)
    alignedUnitsFiles(iFile).mouseID = mouseIDs(iFile);
end
