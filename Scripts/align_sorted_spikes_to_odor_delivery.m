...
% manual curation already applied), align the spiking activity with
% odor delivery, and statistically assess for odor tuning 
%
%   Inputs:
%       (1) [IDxxx]_full_session_odor_sequence_matrix.mat (containing
%       "odorSeqMat" variable and others)
%       
%       (2) [IDxxx]_odors.mat (containing "odorIdentities" variable)
%       
%       (3) [IDxxx]_quality_phy_unit_data_[parameter-details].mat (containing
%       "goodUnitDataStruct" variable)
%           Note: in Version 1.0 of this script, the analogous variable was
%           the "goodUnitsTable" variable, with the following column
%           structure:
%               nCluster = size(goodUnitsTable,1);
%               spikePeaksCell = goodUnitsTable{:,"peak (yes/no)"};
%               getChannelNumber = goodUnitsTable{:,8};
%               clusterID = cell2mat(goodUnitsTable{:,9});
%
%
%   Outputs:
%       ...
...
[file, path] = uigetfile(".mat","Select .mat file containing odorant identities.",baseDir);
load(fullfile(path,file),"odorIdentities");

% load spike variable
[file, path] = uigetfile(".mat","Select .mat file containing goodUnitDataStruct variable for " + mouseLabel + ".",baseDir);
tic
disp("Loading good units data structure...")
load(fullfile(path,file),"goodUnitDataStruct"); % note: took <3 mins
toc

% load anatomical labels for probe contacts
[file, path] = uigetfile(".mat","Select .mat file containing channelToAnatomyTable variable for " + mouseLabel + ".",baseDir);
tic
disp("Loading channelToAnatomyTable structure...")
load(fullfile(path,file),"channelToAnatomyTable"); % note: took <3 mins
toc

% get paths for output figures
baseOutputPath = uigetdir(baseDir,"Select base directory in which to save outputs.");
if saveImages == true
    %savedirSpikeRasters = uigetdir(basePath,"Select destination for peristimulus spike rasters.");
    %savedirPSTH = uigetdir(basePath,"Select destination for PSTHs.");
    %savedirRawCounts = uigetdir(basePath,"Select destination for plotting raw spike counts before and during odor delivery.");
    %savedirAbsoluteChange = uigetdir(basePath,"Select destination for absolute change in spike count figures.");
    %savedirAbsoluteChangeBar = uigetdir(basePath,"Select destination for bar graphs of absolute change in spike count.");
    %savedirPercentChange = uigetdir(basePath,"Select destination for figures plotting percent change in spike count.");
    %savedirPercentChangeBar = uigetdir(basePath,"Select destination for bar graph of percent change in spike count.");
...
%savedirStatisticalTests = uigetdir(basePath,"Select destination for statistical test p-value matrices.");
...
nSamples = goodUnitDataStruct.UNIT001.info.nSamples;
...
for iUnit = 1:nUnits
    spikeIndices = goodUnitDataStruct.("UNIT" + num2str(iUnit, "%.3d")).eventData.spikeIndices;
    spikePeaks(iUnit,spikeIndices) = 1;
end

% Get probe contact numbers
for iUnit = 1:nUnits
    getContactNumber(iUnit) = goodUnitDataStruct.("UNIT" + num2str(iUnit, "%.3d")).info.contactNumb;
end
if iscell(getContactNumber)
    getContactNumber = cell2mat(getContactNumber);
end

clear goodUnitDataStruct
...
save(savedirStatisticalTests + "\" + mouseLabel + "" + "_spikes_grouped_by_odor_trial.mat","spikeRaster", "mouseLabel","timeBefore","timeDuring","timeAfter","sampFreq","getContactNumber","channelToAnatomyTable",'-v7.3');
toc
...
if saveImages == true
    ...
    for iUnit = 1:nUnits
        ...
        sgtitle({"Mouse " + mouseLabel, "sorted & curated", "Unit #" + num2str(iUnit), "Ch#" + thisContactNum + ", " + channelToAnatomyTable.('Anatomical Region')(channelToAnatomyTable.Channel==thisContactNum)})
...
for iUnit = 1:nUnits
    ...
    thisContactNum = getContactNumber(iUnit);
    f = figure('visible','off');
    sgtitle({"Mouse " + mouseLabel, "sorted & curated", "Unit #" + iUnit, "Ch#" + thisContactNum + ", " + channelToAnatomyTable.('Anatomical Region')(channelToAnatomyTable.Channel==thisContactNum)})
...
for iPair = 1:height(tTestSignificant)
    ...
    thisContactNum = getContactNumber(unit);
    tTestSignificant.("Anatomical Location")(iPair) = channelToAnatomyTable.('Anatomical Region')(channelToAnatomyTable.Channel==thisContactNum);
end
...
for iPair = 1:height(wrsTestSignificant)
    ...
    thisContactNum = getContactNumber(unit);
    wrsTestSignificant.("Anatomical Location")(iPair) = channelToAnatomyTable.('Anatomical Region')(channelToAnatomyTable.Channel==thisContactNum);
end
...
for iPair = 1:height(wsrTestSignificant)
    unit = wsrTestSignificant.("Unit #")(iPair);
    thisContactNum = getContactNumber(unit);
    wsrTestSignificant.("Anatomical Location")(iPair) = channelToAnatomyTable.('Anatomical Region')(channelToAnatomyTable.Channel==thisContactNum);
end
...
for iPair = 1:height(signTestSignificant)
    ...
    thisContactNum = getContactNumber(unit);
    signTestSignificant.("Anatomical Location")(iPair) = channelToAnatomyTable.('Anatomical Region')(channelToAnatomyTable.Channel==thisContactNum);
end
...
tic
% Save p-values
save(savedirStatisticalTests + "\" + mouseLabel + "_sortedspikes_curated_signtestresults", "signTestPValues","getContactNumber",'-v7.3');
save(savedirStatisticalTests + "\" + mouseLabel + "_sortedspikes_curated_WRStestresults", "wilcoxanRankSumTestPValues","getContactNumber",'-v7.3');
save(savedirStatisticalTests + "\" + mouseLabel + "_sortedspikes_curated_WSRtestresults", "wilcoxanSignedRankTestPValues","getContactNumber",'-v7.3');
save(savedirStatisticalTests + "\" + mouseLabel + "_sortedspikes_curated_pairedTtestresults", "pairedTTestPValues","getContactNumber",'-v7.3');
